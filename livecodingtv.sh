...
# Copyright (c) 2013-2015, Giovanni Dante Grazioli (deroad), Michal Takac (xunilman)
...
# Streaming Options
OUTRES="1280x720"     # LivecodingTV Output Resolution
...
CBR="2000k"           # Constant bitrate (CBR) Increase this to get a better pixel quality (1000k - 3000k for LivecodingTV)

# Webcam Options
WEBCAM="/dev/video0" # WebCam device (if not working, try /dev/video1)
...
FILE_VIDEO="My_stream.flv"  # File name

# STREAM KEY
# You can find YOUR key here: http://www.LivecodingTV.tv/broadcast/ (Show Key button)
# Save your key inside the livecodingtv_key file
# Or make a global file named ".livecodingtv_key" in your home directory (~/.livecodingtv_key)

# LivecodingTV Server Settings that you can see at https://www.livecoding.tv/YOUR_NICKNAME/settings/channel/
SERVER="eumedia1"             # EU server
PORT="1935"                   # Port number
LTV_USERNAME="YOUR_USERNAME"  # Your LivecodingTV username
...
# LivecodingTV says it MUST have a 44100 rate, please do not change it unless you know what you are doing.
AUDIO_RATE="44100"
# LivecodingTV says it MUST be 2, please do not change it unless you know what you are doing.
KEY_FRAME="2"
...
# Find stream key
if [ -f ~/.livecodingtv_key ]; then
    ECHO_LOG=$ECHO_LOG"\nUsing global LivecodingTV key located in home directory"
    STREAM_KEY=$(cat ~/.livecodingtv_key)
else
    if [ -f ./livecodingtv_key ]; then
        ECHO_LOG=$ECHO_LOG"\nUsing LivecodingTV key located in current running directory"
        STREAM_KEY=$(cat ./livecodingtv_key)
    else
        echo "Could not locate ~/.livecodingtv_key or livecodingtv_key"
        exit 1
    fi
fi
...
streamWebcam(){
        echo "Webcam found!!"
        echo "You should be online! Check on https://www.livecoding.tv/$LTV_USERNAME (Press CTRL+C to stop)"
        echo " "
        if [ -z "$WEBCAM_XY" ]; then
                # checks to avoid a fail on loading the Webcam
...
saveStreamWebcam(){
        echo "Webcam found!!"
        echo "You should be online! Check on http://livecoding.tv/$LTV_USERNAME (Press CTRL+C to stop)"
        echo " "
        if [ -z "$WEBCAM_XY" ]; then
                # checks to avoid a fail on loading the Webcam
                #standard position is: main_w - overlay_w - 10:10
...
doDefaults(){
	if [ $ALWAYS_FULLSCREEN = true ]; then
		...
		rm -f LivecodingTV_tmp 2> /dev/null
		xwininfo -stats >> LivecodingTV_tmp
		TOPXY=$(cat LivecodingTV_tmp | awk 'FNR == 8 {print $4}')","$(cat LivecodingTV_tmp | awk 'FNR == 9 {print $4}')
		INRES=$(cat LivecodingTV_tmp | awk 'FNR == 12 {print $2}')"x"$(cat LivecodingTV_tmp | awk 'FNR == 13 {print $2}')
		rm -f LivecodingTV_tmp 2> /dev/null
		echo " "
	fi
}
...
echo " "
echo "LivecodingTV Streamer for Linux ("$SCRIPT_NAME")"
echo "Forked from Twitch Streamer for Linux: https://github.com/wargio/Twitch-Streamer-Linux"
echo "Copyright (c) 2013 - 2015, Giovanni Dante Grazioli (deroad), Michal Takac (xunilman)"
...
if [ $# -ge 1 ]; then
	...
	if [ $ARG == "-h" ]; then
		...
		echo "[+] Saving the video into $FILE_VIDEO instead of stream to https://www.livecoding.tv/$LTV_USERNAME"
		STREAM_SAVE=1
	elif [ $ARG == "-quiet" ]; then
		if [ $SUPPRESS_OUTPUT = true ]; then
		     continue
